...
const { OnReq, Oc, Pn, Te, Tu, On } = require('../responses/on_req')
...
class WS2Server extends Server {
  ...
  _onClientMessage (ws, msgJSON) {
    ...
    return this._handleUnknownMessage(ws, msg)
  }
  ...
  _handleSubscribeMessage (ws, msg) {
    ...
    this._setChannel(msg.chanId, msg.symbol)
  ...
  _handleNewOrder (ws, msg) {
    let trade
    const ticker = this.ticker[msg[3].symbol]
    const id = this.order.length + 100

    if (msg[3].type === 'MARKET') {
      trade = new Trade(id, msg[3].cid, ticker[6], msg[3].symbol, msg[3].amount, msg[3].flags, msg[3].type)
      this._setAndSend(ws, 'on.res', OnReq(trade))
      this._setAndSend(ws, 'oc.res', Oc(trade))
      this._setAndSend(ws, 'pn', Pn(trade))
      this._setAndSend(ws, 'te', Te(trade))
      this._setAndSend(ws, 'tu', Tu(trade))
    } else if (msg[3].type === 'STOP') {
      trade = new Trade(id, msg[3].cid, msg[3].price, msg[3].symbol, msg[3].amount, msg[3].flags, msg[3].type)
      this._setAndSend(ws, 'on.res', OnReq(trade))
      this._setAndSend(ws, 'on', On(trade))
    } else if (msg[3].type === 'TRAILING STOP') {
      trade = new Trade(id, msg[3].cid, msg[3].price, msg[3].symbol, msg[3].amount, msg[3].flags, msg[3].type)
      this._setAndSend(ws, 'on.res', OnReq(trade))
      this._setAndSend(ws, 'on', On(trade))
    }

    this.order.push(trade)
  ...
  _setChannel (chanId, symbol) {
    this.channel[symbol] = chanId
  }

  _setAndSend (ws, code, msg) {
    this._responses.set(code, {
      'packets': msg
    })
    this._sendResponse(code, ws)
  }

  setTicker (symbol, ticker) {
    const chanId = this.channel[symbol]
    this.ticker[symbol] = ticker
    this._responses.set('ticker', {
      'packets': [[chanId, ticker]]
    })
    this._sendResponse('ticker', this._clients[0])
  }
}
