...
const { OnReq, Oc, Pn, Te, Tu } = require('../responses/on_req')
...
class WS2Server extends Server {
  ...
  _handleSubscribeMessage (ws, msg) {
    ...
    this._sendResponse('ticker', ws)

    debug('client subscribed to channel %s', msg.channel)
  }

  _handleNewOrder (ws, msg) {
    this._responses.set('on.res', {
      'packets': OnReq(msg)
    })
    this._sendResponse('on.res', ws)

    this._responses.set('oc.res', {
      'packets': Oc(msg)
    })
    this._sendResponse('oc.res', ws)

    this._responses.set('pn', {
      'packets': Pn(msg)
    })
    this._sendResponse('pn', ws)

    this._responses.set('te', {
      'packets': Te(msg)
    })
    this._sendResponse('te', ws)

    this._responses.set('tu', {
      'packets': Tu(msg)
    })
    this._sendResponse('tu', ws)
    const o = msg[3]
